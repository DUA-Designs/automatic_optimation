...
if($_POST['interested']!=NULL){ $interested=$_POST['interested']; }
...
<tr bgcolor='ffffff'>
<td height='40' style='text-align:center;'> Enquiry For Evora </td>
</tr>
<tr>
<td>
<table border='0' width='560' align='center' cellpadding='0' cellspacing='0' class='container-middle'>
...
CURLOPT_POST => true,
CURLOPT_POSTFIELDS => [
'secret' => '6LfgMN8qAAAAAB5YDZjXYQN2pCF6ikiOQMBq9czR',
'response' => $_POST["token"],
...
$mail = new PHPMailer();		
$mail->From      = 'info@bhavyaconstructions.com';
$mail->FromName  = "TD-G Rank-Enquiry";		
$mail->Subject   = $subject;		
$mail->Body      = $message;		
$mail->SetFrom( "info@bhavyaconstructions.com", 'TD-G Rank-Enquiry');

$mail->AddAddress("divyagone@grank.co.in", "Evoravillas");
// $mail->AddAddress("enquiry@bhavyaconstructions.com", "Evoravillas");

// $mail->AddBCC("leadtest@grank.co.in", "Evoravillas");
// $mail->AddBCC("maheshwari@grank.co.in", "Evoravillas");
// $mail->AddBCC("srujana@grank.co.in", "Evoravillas");
...
$mail->IsHTML(true);
  ...
  if($mail->send())
  {
    ...
    $leaddata = array (
        "sell_do" => array(
            "analytics" => array("utm_campaign" => '', "utm_source"=>''),
            "campaign" => array("srd" => $SRD_val),
            "form" => array(
                "custom" => array(
                  "custom_looking_for_a_luxury_four_bhk_villa_in_kollur"=>$interested,
                  "custom_budget_starts_at_four_and_half_cr_interested"=>$budget
                ),  
                "lead" => array ("name" =>$name, "phone" =>$phone,"email"=>$email)
            )
        ),
        "api_key" =>'0592a0fe85fc20ffb34b59d27a52e776'
    );
    ...
    curl_close($ch);
  echo $response;
//  header("Location: thank-you.php?email=" . urlencode($email));
  }
